	...
	function process_advanced_fonts_options( $function_name ) {
		...
		foreach ( $this->advanced_fields['fonts'] as $option_name => $option_settings ) {
			...
			if ( isset( $font_options[ $line_height_option_name ] ) ) {
				...
				$line_height_option_value = $font_options[ $line_height_option_name ];

				if ( $is_font_size_set || ! in_array( trim( $line_height_option_value ), array( '', 'px', $default_line_height ) ) ) {
					$important = in_array( 'line-height', $important_options ) || $use_global_important ? ' !important' : '';

					if ( empty( $line_height_option_value ) ) {
						$line_height_option_value = $default_line_height;
					}

					$line_height_value = et_builder_process_range_value( $line_height_option_value, 'line_height' );
